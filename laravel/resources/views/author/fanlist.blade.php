...
@section('content')
    <div class="row">
        ...
        @if($fans_info==null)
            <div class="alert alert-warning" role="alert">你目前还没有粉丝，请多多发表文章来增加人气</div>
        @else
            @foreach($fans_info as $fan)
                <a href="{{asset('author/').'/'.$fan->name}}" style="margin-right: 10px"><img class="head_ico"
                                                                                              src="{{$fan->head_ico}}"></a><span>{{$fan->name}}</span>
            @endforeach
        @endif
        </div>
